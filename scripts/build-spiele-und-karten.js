#!/usr/bin/env node
/*
 * Baut für IT Lernfeld 1 aus Datendateien + gemeinsamen Templates:
 *   - Karteikarten_<slug>.html   (aus scripts/data/karteikarten/session-N.json)
 *   - Memory_<slug>.html         (Paare aus denselben Karteikarten-Daten)
 *   - Wordle_<slug>.html         (aus scripts/data/spiele/session-N.json)
 *   - Hangman_<slug>.html        (aus scripts/data/spiele/session-N.json)
 *
 * Nutzung:
 *   node scripts/build-spiele-und-karten.js            # alle Sessions
 *   node scripts/build-spiele-und-karten.js session-3  # nur Session 3
 *
 * Datenschema karteikarten/session-N.json:
 *   { session, thema, slug, day_id, cards: [ { topic, front, back }, ... ] }  (mind. 20 Karten)
 ...
 *
 * Die Templates enthalten "Tag {{TAG}}"; für IT Lernfeld 1 wird daraus "Session {{TAG}}".
 */
const fs = require('fs');
...
const COURSE = ROOT;
const KART_DIR   = path.join(ROOT, 'scripts', 'data', 'karteikarten');
const SPIELE_DIR = path.join(ROOT, 'scripts', 'data', 'spiele');
...
const bySession = Object.fromEntries(SESSIONS.map(s => [s.nr, s]));

const EXPECT_CARDS = 20;
const MIN_WORDS = 60;
const MIN_PAIRS = 8;
...
function fill(tpl, { tag, thema, day_id, jsonKey, json }) {
  return tpl
    ...
    .replaceAll('Büroprozesse (M-067)', 'IT Lernfeld 1')
...
for (const file of kartFiles) {
  ...
  if (!Array.isArray(cards)) problems.push('cards kein Array');
    ...
    if (cards.length < EXPECT_CARDS) problems.push(`${cards.length} Karten (erwartet mind. ${EXPECT_CARDS})`);
    const seen = new Set();
    cards.forEach((c, i) => {
  ...
  const sess = bySession[session];
  const outFolder = path.join(COURSE, folderOf(sess));
...
console.log(errors ? `\n❌ ${errors} Problem(e)` : '\n✅ IT Lernfeld 1: alle Spiele/Karten/Memory gebaut');
